"""Store D transfer. Mock LLM proves plumbing only — labels are Japanese."""
...
PAGE = ROOT / "web" / "app" / "store-d" / "page.tsx"
ADAPTER_D = ROOT / "adapters" / "store_d__publish_product.json"
ADAPTER_B = ROOT / "adapters" / "store_b__publish_product.json"


@pytest.fixture(autouse=True)
def _store_d_env(monkeypatch):
    monkeypatch.setenv("SKILLSHIFT_MOCK_LLM", "1")
    monkeypatch.setenv("SKILLSHIFT_TARGET_APP", "store-d")


def test_store_b_does_not_read_store_d_adapter(monkeypatch):
    ...
    assert path != ADAPTER_D


def test_store_d_visible_copy_has_no_ascii_letters():
    """Source lint only. Mock transfer below does not prove Japanese grounding."""
...
def test_store_d_cold_start_then_cache_hit():
    ...
    with open_hands() as hands:
        ...
        adapter, _ = run_store_b_transfer(
            hands, skill, LEATHER_BAG, empty_adapter("store-d"), persist_adapter=True
        )
        first = snapshot()
    ...
    assert publish.resolved_targets == ["store-d-publish"]

    persisted = json.loads(ADAPTER_D.read_text(encoding="utf-8"))
    assert persisted["app_id"] == "store-d"
