...
Note for 7F: dashboard does not render perturbation flags; they live on `/store-b` judge controls. Store C/D/E appear on the home page; the dashboard app card stays 7F-owned.

Follow-on: Store E (`/store-e`) is a Farsi RTL twin of B — same frozen Skill, finish `store-e-release`.
